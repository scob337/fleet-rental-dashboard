...
import { ArrowLeft, Users, Truck, CalendarDays, DollarSign, List } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 page-enter">
      <div className="flex items-center justify-between gap-4">
        <div>
          <Link href="/dashboard/drivers" className="inline-flex items-center gap-2 text-sm text-primary hover:text-primary-foreground transition-colors">
            <ArrowLeft size={16} /> العودة إلى السائقين
          </Link>
          <h1 className="mt-4 text-3xl font-bold flex items-center gap-3">
            <Users size={28} /> {driver.name}
          </h1>
          <p className="text-sm text-muted-foreground mt-1">رقم الرخصة {driver.licenseNumber}</p>
        </div>
        <div className="rounded-3xl border border-border bg-card p-5 text-right">
          <p className="text-xs text-muted-foreground">حالة السائق</p>
          <p className="mt-2 text-lg font-semibold">{driver.status}</p>
          <p className="text-sm text-muted-foreground mt-2">عدد سنوات الخبرة: {driver.experience ?? 0}</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-3xl border border-border bg-card p-5">
          ...
          {driver.tripLogs.length === 0 ? (
            <p className="text-sm text-muted-foreground">لا توجد رحلات مسجلة لهذا السائق بعد.</p>
          ) : (
              ...
              <table className="min-w-full text-right text-sm">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="p-3 text-xs text-muted-foreground">الإيراد</th>
                  </tr>
                </thead>
                <tbody>
                  {driver.tripLogs.map((trip) => (
                    <tr key={trip.id} className="border-b border-border/70 hover:bg-secondary/50 transition-colors">
                      <td className="p-3">{trip.vehicle?.plateNumber ?? 'غير محدد'}</td>
        ...
        <div className="rounded-3xl border border-border bg-card p-5">
          <h2 className="font-semibold text-lg mb-4">تفاصيل السائق</h2>
          <div className="space-y-3 text-sm text-slate-700">
            <div><span className="font-semibold">رقم الجوال:</span> {driver.phone ?? '-'}</div>
            <div><span className="font-semibold">البريد الإلكتروني:</span> {driver.email ?? '-'}</div>
            <div><span className="font-semibold">رخصة الصف:</span> {driver.licenseClass ?? '-'}</div>
            <div><span className="font-semibold">انتهاء الرخصة:</span> {formatDate(driver.licenseExpiry)}</div>
            <div><span className="font-semibold">عدد الرحلات الكلي:</span> {totalTrips}</div>
            <div><span className="font-semibold">إجمالي الإيراد:</span> {formatCurrency(totalRevenue)}</div>
            <div><span className="font-semibold">السيارة المرتبطة:</span> {driver.vehicle?.plateNumber ?? 'غير مرتبط'}</div>
          </div>
        </div>
      </div>
    </div>
